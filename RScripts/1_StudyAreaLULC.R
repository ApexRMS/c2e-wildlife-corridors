...
studyAreaPoints <- read_csv(
				file.path(paste0(dataDir, "/Study Area"), 
				"Vertices 20200610 DAG.csv")) # Study area vertices
  # LULC data
SOLRISRaw <- raster(
				file.path(paste0(dataDir,"/Land use land cover/SOLRIS_Version_3_0"), 
				"SOLRIS_Version_3_0_LAMBERT.tif"))
OLCDBRaw <- raster(
				file.path(paste0(dataDir, "/Land use land cover/Provincial Land Cover 1996 - 28 Class Grid"), 
				"plc1996_28.tif"))
ORNRaw <- st_read(
				file.path(paste0(dataDir, "/Land use land cover/Ontario_Road_Network__ORN__Segment_With_Address-shp"), 
				"Ontario_Road_Network__ORN__Segment_With_Address.shp"))
OHNRaw <- st_read(
				file.path(paste0(dataDir, "/Land use land cover/Ontario_Hydro_Network__OHN__-_Waterbody-shp"),
				"Ontario_Hydro_Network__OHN__-_Waterbody.shp"))
urbanRaw <- st_read(
				file.path(paste0(dataDir, "/Land use land cover/Built-Up_Area-shp"), 
				"Built-Up_Area.shp"))
  
  # Resistance crosswalk
crosswalk <- read_csv(
				file.path(paste0(dataDir, "/Resistance"), 
